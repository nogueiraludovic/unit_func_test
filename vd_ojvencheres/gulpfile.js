...
const clean = () => { return deleteAsync(PATHS.scripts.destination + '/vd-ojvencheres.min.js') }
...
const scripts = () => {
  return src(PATHS.scripts.source)
    ...
    .pipe(terser())
    .pipe(concat('vd-ojvencheres.min.js'))
    .pipe(condition(PROD === false, sourcemaps.write('', {})))
    .pipe(dest(PATHS.scripts.destination))
}

const watcher = () => { watch(PATHS.scripts.source, scripts) }

export const build = series(clean, parallel(scripts))
export const development = series(parallel(scripts), watcher)
