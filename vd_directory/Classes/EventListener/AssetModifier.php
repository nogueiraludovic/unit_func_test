...
namespace Vd\VdDirectory\EventListener;
...
final class AssetModifier
{
    public function __invoke(AfterInitializeActionEvent $event): void
    {
        ...
        if ($cssForColor !== '') {
            ...
            $assetCollector->addStyleSheet('vd-directory', $this->writeStyleSheetContentToTemporaryFile($cssForColor));
        }

        $assetCollector->addJavaScript(
            'vd-directory',
            'EXT:vd_directory/Resources/Public/JavaScript/vd-directory.min.js'
        );
    }
    ...
    protected function getCssForColor(): string
    {
        ...
        foreach ($colors as $color) {
            ...
            $css .= '.vd-directory .card-' . $uid . ':after {'
